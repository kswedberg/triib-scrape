...
const getWorkoutTitle = (title, level) => {
  const t = level === 'Rx+' ? `${title} Rx+` : title;

  return t.replace(regexes.cleanRM, '');
};
const getDate = (d) => {
...
const buildRows = async({startAt, endAt, memberType, dirs}) => {
  ...
  members.forEach((member, i) => {
    const pct = `${Math.round((i / len) * 100)}% ${memberType} prepped`;
...
const outputFile = ({rows, expunge, memberType, dirs}) => {
  ...
  console.log(chalk.cyan(`${rows.length} gathered. Saving to ${saveTo}\n`));
